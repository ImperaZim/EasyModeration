...
class Punish extends Punishment {
 ...
 public function getTime() : Int {
  $query = SQLite3::table()->query("SELECT * FROM banned WHERE name='" . SQLite3::table()->escapeString($this->content["player"]) . "';");
 ...
 public function absolve() : bool {
  ...
  if ($process == 0) {
   if ($this->isBanned($this->content["player"])) {
    SQLite3::table()->query("DELETE FROM banned WHERE name='" . SQLite3::table()->escapeString($this->content["player"]) . "';");
   }
  }
  return $process == 0;
 }
